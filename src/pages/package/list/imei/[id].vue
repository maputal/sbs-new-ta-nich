...
const fetchImei = async () => {
  ...
  try {
    ...
    appStore.showError(error.message || 'Failed to load IMEI data')
  } finally {
    appStore.hideLoader()
  }
}
...
const deleteImei = (item = null) => {
  ...
  if (item) {
    ...
    appStore.setPopup({
      ...
      word: 'Are you sure you want to delete this IMEI record?',
      action: '2',
    ...
    appStore.setPopup({
      title: 'Confirm Deletion',
      word: 'Are you sure you want to delete all selected IMEI records?',
      action: '2',
      onSucc: confirmDelete,
    })
  }
...
const confirmDelete = async () => {
  ...
  try {
    ...
    appStore.setPopup({
      title: 'Success!',
      word: 'IMEI record(s) deleted successfully',
      action: 'success',
      onSucc: () => {},
    })
    itemToDelete.value = null
  } catch (error) {
    appStore.showError(error.message || 'Failed to delete IMEI record(s)')
  } finally {
    appStore.hideLoader()
  }
}
