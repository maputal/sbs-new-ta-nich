...
const fetchMsisdn = async () => {
  try {
    ...
    successMessage.value = "MSISDN data loaded successfully"
    isSuccessToastVisible.value = true
  } catch (error) {
    errorMessage.value = error.message || "Failed to load MSISDN data"
    isErrorToastVisible.value = true
  }
}

const deleteMsisdn = (item = null) => {
  if (item) {
    itemToDelete.value = item
    confirmMessage.value = `Are you sure you want to delete this MSISDN record?`
  } else {
    confirmMessage.value = `Are you sure you want to delete all selected MSISDN records?`
  }
...
const confirmDelete = async () => {
  try {
    ...
    successMessage.value = "MSISDN record(s) deleted successfully"
    isSuccessToastVisible.value = true
    itemToDelete.value = null
  } catch (error) {
    errorMessage.value = error.message || "Failed to delete MSISDN record(s)"
    isErrorToastVisible.value = true
  }
}
...
const uploadMsisdn = () => {
  router.push(`/package/list/msisdn/upload/${packageId}`)
}
...
<template>
  ...
  <VCard>
    ...
    <VCardText>
      <ListUploader
        type="msisdn"
        :fetch-function="fetchMsisdn"
        :delete-function="deleteMsisdn"
        :upload-function="uploadMsisdn"
      />
    </VCardText>
  </VCard>
</template>
